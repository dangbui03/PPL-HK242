...
class CodeGenVisitor(BaseVisitor):
    ...
    def visitVarDecl(self, ast, o):
        ...
        if ast.varType is not None:
            ...
            for item in Symbol:
                for sym in item:
                    if sym.name == ast.name.name:
                        typ = sym.typ
                        break
        
        index = frame.getNewIndex()
        self.emit.printout(self.emit.emitVAR(index, ast.name.name, typ, frame.getStartLabel(), frame.getEndLabel(), frame))
        Symbol[0].append(VarZcode(ast.name.name, ast.varType, index, True if ast.varInit else False))
        Symbol[0][-1].line = self.emit.printIndexNew()
        
        if ast.varInit is not None:
            self.visit(Assign(ast.name, ast.varInit), o)
            codeInit, typeInit = self.visit(ast.varInit, o)
            self.emit.printout(self.emit.emitVAR(index, ast.name.name, typeInit, frame.getStartLabel(), frame.getEndLabel(), frame))
        elif type(ast.varType) is ArrayType:
            if len(ast.varType.size) == 1:
                ...
                self.emit.printout(self.emit.emitPUTSTATIC(self.className + "." + ast.name.name, ast.varType, frame))
            else:
        ...
        for item in Symbol:
            for sym in item:
                if sym.name == ast.name:
                    ...
                    if index == -1:
                        if o.isLeft == True:
                            return self.emit.emitPUTSTATIC(self.className + "/" + ast.name, sym.typ, frame), sym.typ
                        return self.emit.emitGETSTATIC(self.className + "/" + ast.name, sym.typ, frame), sym.typ
                    else:
                        if o.isLeft == True:
                            return self.emit.emitWRITEVAR(ast.name, sym.typ, index, frame), sym.typ
                        return self.emit.emitREADVAR(ast.name, sym.typ, index, frame), sym.typ
    ...
    def visitBlock(self, ast, o):
        symbolnew = [[]] + o.ysmbol #! tăng tầm vực
    ...
    def visitIf(self, ast, o):
        ...
        if_label = frame.getNewLabel()
        list_label_elif = [frame.getNewLabel() for _ in ast.elifStmt]
        self.emit.printout(self.emit.emitLABEL(label_exit, o.frame))  
        
        #TODO: implement
        exprCode, _ = self.visit(ast.expr, Access(frame, o.symbol, False))
        self.emit.printout(exprCode)
        self.emit.printout(self.emit.emitIFTRUE(if_label, frame))
        self.visit(ast.thenStmt, o)
        self.emit.printout(self.emit.emitGOTO(label_exit, frame))
        self.emit.printout(self.emit.emitLABEL(if_label, frame))
        if ast.elifStmt:
            for item in ast.elifStmt:
                if_label = frame.getNewLabel()
                frame.enterLoop()
