...
class FunctionType(Type):
    ...
    def __str__(self):
        return "FunctionType(" + self.name + ",[" +  ','.join(str(i) for i in self.params) + "]," + str(self.retType) + ")"
...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is BoolType: #* kiểu bool
            return "Z"
        elif typeIn is IntType: #* kiểu int
            return "I"
        elif typeIn is FloatType: #* kiểu float
            return "F"
        ...
        elif typeIn is FunctionType: #* có 2 giống ý tưởng VarZcode
            return "(" + "".join(list(map(lambda x: self.getJVMType(x), inType.params))) + ")" + self.getJVMType(inType.retType)
